...
router.post("/placeorder", async(req, res) => {
  ...
  try {
      ...
      if(payment)
      {
          ...
          res.send('Sipariş Başarıyla Oluşturuldu!')
      }
      else{
          res.send('Ödeme Hatası!')
      }

  } catch (error) {
      return res.status(400).json({ message: 'Bir Sorun Oluştu!' + error});
  }
...
router.post("/getuserorders", async(req, res) => {
  ...
  try {
      ...
      return res.status(400).json({ message: 'Bir Sorun Oluştu!' });
  }
});
